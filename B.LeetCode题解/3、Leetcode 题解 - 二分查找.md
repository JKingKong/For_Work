...
public char nextGreatestLetter(char[] letters, char target) {
    ...
    int l = 0, h = n - 1;
    while (l <= h) { 
        // l == h继续循环，即：寻找的字符在数组中不存在，且被两个字符夹着的情况，比如上述的实例一
        // letters[h] < target, letter[l] < target,letter[m] < target
        int m = l + (h - l) / 2;
        if (letters[m] <= target) {
            // 当前字符小于等于目标
            l = m + 1;
...
在 index 之后，数组中原来存在的成对状态被改变。如果 m 为偶数，并且 m + 1 < index，那么 nums[m] == nums[m + 1]；m + 1 >= index，那么 nums[m] != nums[m + 1]。
...
public int singleNonDuplicate(int[] nums) {
    ...
    while (l < h) {
        ...
        if (nums[m] == nums[m + 1]) { // m是偶数，m+1是奇数，   0,0,2,2,3,5,5
            // 前半部分并未被破坏成对状态
            //说明index在 [m+2,h]
            l = m + 2;
        } else {
            // 前半部分出现single element，破坏成对出现的状态
            // 说明index在[l,m]上
            h = m;
        }
    }
...
题目描述：给定一个元素 n 代表有 [1, 2, ..., n] 版本，在第 x 位置开始出现错误版本，导致后面的版本都错误。可以调用 i**sBadVersion(int x)** **（题目给的内置函数）**知道某个版本是否错误，要求找到第一个错误的版本。
...
public int[] searchRange(int[] nums, int target) {
    ...
    int last = findLeft(nums, target + 1) - 1;
    if (first == nums.length || nums[first] != target) { 
        // first == nums.length 说明该数比数组中的所有数都要大
        // nums[first] != target 说明目标元素不在数组中，查找失败，如果该数比数组中的所有元素都要小的话，返回的是first == last == 0, nums[first] != target 
        return new int[]{-1, -1};
    } else {
...
在寻找第一个位置的二分查找代码中，需要**注意 h 的取值为 nums.length**，而不是 nums.length - 1。先看以下示例：

```
nums = [2,2], target = 2
```

如果 h 的取值为 nums.length - 1，那么 last = findLeft(nums, target + 1) - 1 = 1 - 1 = 0。这是因为 findLeft 只会返回 [0, nums.length - 1] 范围的值，对于 findLeft([2,2], 3) ，我们希望返回 3 插入 nums 中的位置，也就是数组最后一个位置再往后一个位置，即 nums.length。所以我们需要将 h 取值为 nums.length，从而使得 findLeft返回的区间更大，能够覆盖 target 大于 nums 最后一个元素的情况。
